...
public class LifeGameFrame extends javax.swing.JFrame 
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        // TODO add your handling code here:
        int rndCol = (int) (Math.random()*gridCount);
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        ...
        for (int col=1;col<gridCount-1;col++) {
            for (int row=1;row<gridCount-1;row++) {
                if (temp[col][row] == 0) {
                    if (temp[col-1][row-1] == 1) count++; //top left
                    if (temp[col-1][row] == 1) count++; // left
                    if (temp[col-1][row+1] == 1) count++; //bottom left
                    if (temp[col][row+1] == 1) count++; //bottom
                    if (temp[col+1][row+1] == 1) count++; //bottom right
                    if (temp[col+1][row] == 1) count++; //right
                    if (temp[col+1][row-1] == 1) count++; //top right
                    if (temp[col][row-1] == 1) count++; // top
                    if (count == 2) {
                        grid[col][row] = 1;
                    }
                }
    ...
    private void jButton9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton9ActionPerformed
        ...
        for (int col=0;col<gridCount;col++) {
            for (int row=gridCount-1;row>=0;row--) {
                grid[gridCount-row-1][col] = temp[col][row];
            }
        }
